...
phase_40_dotfiles() {
  ...
  bw_link_managed "$BW_REPO_ROOT/dotfiles/local-bin/.local/bin/bw-clipboard-copy" "$HOME/.local/bin/bw-clipboard-copy"
  ...
  if [[ $BW_PLATFORM == wsl && $BW_TEST_MODE != 1 ]] && bw_have cmd.exe && bw_have wslpath; then
